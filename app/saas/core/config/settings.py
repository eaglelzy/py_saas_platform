...
class Settings(BaseSettings):
    ...
    rate_limit_default_window_seconds: int = Field(60, description="限流默认时间窗口（秒）")
    ...
    verification_code_ttl_seconds: int = Field(5 * 60, description="验证码有效期（秒）")
    verification_code_max_attempts: int = Field(5, description="验证码最大校验次数")
    verification_code_length: int = Field(6, description="验证码长度")
